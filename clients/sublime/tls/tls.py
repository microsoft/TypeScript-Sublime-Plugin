...
class Client:
    def __init__(self):
        ...
        procFile=settings.get('tls_proc_file')
        if not procFile:
...
# per-file info that will only be accessible from TlsListener instance
class FileInfo:
...
class tlsListener(sublime_plugin.EventListener):
    def __init__(self):
        self.fileMap={}
    ...
    def update_status(self,view,info):
        if info.hasErrors:
            view.run_command('tls_error_info')
        else:
            view.erase_status("tls_error")
        errstatus=view.get_status('tls_error')
        if errstatus and (len(errstatus)>0):
            view.erase_status("tls_info")
        else:
            view.run_command('tls_quick_info')
    ...
    def on_post_text_command(self,view,command_name,args):
        ...
        if info:
            if (not info.changeSent) and info.modified:
                ...
                elif ((command_name=="tls_format_on_key")or (command_name=="tls_format_document") or
                      (command_name=="tls_format_selection") or (command_name=="tls_format_line")):
...
class TlsQuickInfo(sublime_plugin.TextCommand):
    def handleQuickInfo(self,data):
        ...
        if data['success']:
            ...
            self.view.set_status("tls_info",infoStr)
        else:
            self.view.erase_status("tls_info")
...
class TlsQuickInfoDoc(sublime_plugin.TextCommand):
    def handleQuickInfo(self,data):
        print(data)
        if data['success']:
            allinfo=data['body']
...
class TlsErrorInfo(sublime_plugin.TextCommand):
    def run(self,text):
        ...
        for (region,text) in clientInfo.errors['semanticDiag']:
            if region.contains(pt):
                errorText=text
        if len(errorText)>0:
            self.view.set_status("tls_error",errorText)
        else:
            self.view.erase_status("tls_error")
                
    def is_enabled(self):
        return is_typescript(self.view)

# go to definition command
class TlsGoToDefinitionCommand(sublime_plugin.TextCommand):
    def run(self,text):
...
# go to type command
class TlsGoToTypeCommand(sublime_plugin.TextCommand):
    def run(self,text):
...
# rename command
class TlsRenameCommand(sublime_plugin.TextCommand):
    def run(self,text):
        ...
        if data['success']:
            ...
            def on_cancel():
                return 
            def on_done(newName):
                self.view.run_command('tls_finish_rename',
...
class TlsFinishRenameCommand(sublime_plugin.TextCommand):
...
class TlsFindReferencesCommand(sublime_plugin.TextCommand):
    def run(self,text):
        ...
        if data['success']:
            ...
            refView=getRefView()
            refView.run_command('tls_populate_refs',
                                {
                                    "refId" : refId,
...
def updateRefLine(refInfo,curLine,nextLine,view):
    ...
    view.add_regions("curref",[sublime.Region(caretPos,caretPos+1)],
                     "keyword","Packages/tls/icons/arrow-right3.png",
                     sublime.HIDDEN)

# if cursor is on reference line, go to (filename, line, col) referenced by that line    
class TlsGoToRefCommand(sublime_plugin.TextCommand):
...
class TlsNextRefCommand(sublime_plugin.TextCommand):
    def run(self,text):
        print("next ref")
        ...
        if refView:
            ...
            if mapping:
                ...
                refView.run_command('tls_go_to_ref')

# FIX: this works in the middle of the ref file but not at the end
# go to previous reference in active references file
# TODO: generalize this to work for all types of references
class TlsPrevRefCommand(sublime_plugin.TextCommand):
    def run(self,text):
        print("prev ref")
        if (self.view.file_name()):
            print(self.view.file_name())
        ...
        if refView:
            ...
            if mapping:
                ...
                refView.sel().clear()
                refView.sel().add(sublime.Region(pos,pos))
                refView.run_command('tls_go_to_ref', { "forward": False })
...
# helper command called by TlsFindReferences; put the references in the
# references buffer
# TODO: generalize this to populate any type of references file 
# (such as build errors)
class TlsPopulateRefs(sublime_plugin.TextCommand):
    def run(self,text,refId="",refDisplayString="",refs=[],line=0,filename=""):
        ...
        self.view.set_syntax_file('Packages/tls/FindRefs.hidden-tmLanguage')
...
class TlsFormatOnKey(sublime_plugin.TextCommand):
