...
class TypescriptGetCodeFixesCommand(TypeScriptBaseTextCommand):
    ...
    def handle_selection(self, idx):
        ...
        start = self.view.text_point(current_code_fix[0]['start']['line'] - 1, current_code_fix[0]['start']['offset'] - 1)
        end = self.view.text_point(current_code_fix[0]['end']['line'] - 1, current_code_fix[0]['end']['offset'] - 1)
        text = current_code_fix[0]['newText']
        self.view.run_command('replace_text', {'start': start, 'end': end, 'text': text})
    ...
    def run(self, text):
        ...
        errors = cli.service.get_errors_at_cursor(self.view.file_name(), cursor)
        ...
        if len(errors):
            ...
            self.all_code_fixes = cli.service.get_code_fixes(self.view.file_name(), start_line, start_offset, end_line, end_offset, error_codes)
            if self.all_code_fixes['success']:
                if len(self.all_code_fixes['body']):
                    possibleFixesDescriptions = list(map(lambda fix: fix['description'], self.all_code_fixes['body']))
                    if len(possibleFixesDescriptions) == 1:
                        self.handle_selection(0)
                    else:
                        self.view.window().show_quick_panel(possibleFixesDescriptions, self.handle_selection, False, -1)
