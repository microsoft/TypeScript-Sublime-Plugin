/// <reference path='/Users/steve/src/TypeScript/src/server/session.ts' />
/// <reference path='/Users/steve/src/TypeScript/src/compiler/sys.ts' />
...
*/
    ...
    function editStress(fname: string, timing: boolean) {
        ...
        console.log("stress " + fname + " " + content.length.toString());
        var lm = LineIndex.linesFromText(content);
        var lines = lm.lines;
        if (lines.length == 0) {
            return;
        ...
        for (j = 0; j < 100000; j++) {
            ...
            if (!timing) {
                ...
                if ((lac.line+1) != lp.line) {
                    recordError();
                    console.log("arrgh " + (lac.line+1) + " " + lp.line + " " + j);
                    return;
                }
                if (lac.character != lp.col) {
                    recordError();
                    console.log("arrgh ch... " + lac.character + " " + (lp.col + 1) + " " + j);
                    return;
                }
            }
        }
        ...
        for (var ko = 0; ko < outer; ko++) {
            for (var k = 0, llen = lines.length; k < llen; k++) {
                ...
                var lineIndexOffset = lineInfo.col;
                if (!timing) {
